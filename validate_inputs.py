...
def main():
    ...
    minimum_conserved_node0_witness = float("inf")
    for row in index["cases"]:
        ...
        fleet_size = float(data["fleet_size"])
        assert fleet_size == float(protocol["fleet_size"])
        assert abs(float(data["predicted_original"].sum()+data["node0_predicted_original"])-fleet_size) < prep.TOL
        assert data["predicted"].sum() <= fleet_size+prep.TOL
        assert abs(float(data["node0_nominal_after_center_shift"])-(fleet_size-data["predicted"].sum())) < prep.TOL
        ...
        expected_global = [max(calibration["statistical_global_band"][0], -float(data["predicted"].sum())),
                           min(calibration["statistical_global_band"][1], fleet_size-float(data["predicted"].sum()))]
        actual_global = [specs["C0"]["groups"][0][key] for key in ("lower", "upper")]
        assert np.allclose(actual_global, expected_global, atol=prep.TOL, rtol=0)
        for arm in prep.ROBUST_ARMS:
            ...
            if arm in ("C0", "C0V"):
                node0 = fleet_size-float(realized.sum())
                minimum_conserved_node0_witness = min(minimum_conserved_node0_witness, node0)
                assert -prep.TOL <= node0 <= fleet_size+prep.TOL
    ...
    report = dict(status="passed", real_network_instances=144, station_count=567,
                  ...
                  node0_physical_bound_verified_arms=["C0", "C0V"],
                  nominal_D_fleet_bound_verified=True,
                  minimum_conserved_node0_witness=minimum_conserved_node0_witness,
