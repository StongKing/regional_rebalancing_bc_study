...
def main():
    ...
    for case in selected:
        ...
        inventory, actual_node0 = snapshots.actual_inventory_snapshot(actual, int(position[0]), data)
        for budget, runs in ((12, main_runs), (60, budget_runs)):
            for arm in ARMS:
                ...
                if abs(replay["node0_inventory"]-actual_node0) > snapshots.TOL:
                    raise AssertionError("Budget replay explicit node0 differs from fleet residual")
