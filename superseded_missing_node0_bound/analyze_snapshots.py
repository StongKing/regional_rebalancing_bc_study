...
def physical_check(routes, inventory, data, cap):
    ...
    violations = [load_violation, endpoint_violation, region_balance_error,
                  inventory_conservation_error, unvisited_handling, negative, overflow]
...
def main():
    ...
    for case in index["cases"]:
        ...
        inventory = np.asarray(actual[int(positions[0]), 1:, 0], float)
        specs = read(directory/"uncertainty_specs.json")
        for arm in ("B", "C0", "C0V"):
    ...
    lines += ["", "上下界针对固定路线的最坏追索，不是整个车辆路径问题的全局最优证明。未闭合的内层不当作精确最坏值；模型内最坏罚损下降，也不能替代实际快照目标偏差改善。", "",
              ...
              f"全部672次实际快照追索通过物理核验；最大数值违例为{summary['physical_checks']['maximum_violation']:.3g}。核验包括未访问站零装卸、库存非负和不超站容、车载量上下界、空车返回、全网库存守恒以及各区净送入等于实际跨区车载量净流。", "",
              "## 数据与解释限制", "",
