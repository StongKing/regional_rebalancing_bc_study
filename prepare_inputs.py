...
OUT = HERE / "inputs"
DATASET_METADATA = HERE.parents[1] / "data/DIVVY/dataset_meta.json"
PREDICTION_ARCHIVE = HERE.parents[1] / "outputs/01a0a96b-0035-7f22-b360-8fc2b56e2ddf/rebalancing_epoch_86/all_predictions.npz"
...
def build_specs(data, metadata, regions):
    ...
    statistical_global_band = [float(global_center-radius*global_scale+shift.sum()),
                              float(global_center+radius*global_scale+shift.sum())]
    fleet_size = float(np.asarray(data["fleet_size"]))
    physical_global_band = [-float(data["predicted"].sum()),
                            fleet_size-float(data["predicted"].sum())]
    global_band = [max(statistical_global_band[0], physical_global_band[0]),
                   min(statistical_global_band[1], physical_global_band[1])]
    if global_band[0] > global_band[1]+TOL:
        raise ValueError("Statistical global band and node0 physical inventory do not intersect")
    ...
    statistical_global_group = dict(global_group, lower=statistical_global_band[0],
                                     upper=statistical_global_band[1])
    ...
    for arm, spec in specs.items():
        ...
        if arm == "D": continue
        # Historical statistical coverage is measured before applying CURRENT
        # station/node0 inventory constraints. Those physical translations are
        # separate from the historical joint calibration guarantee.
        raw_groups = [] if arm == "B" else [statistical_global_group]
        if arm == "C0V": raw_groups += regional_groups
        raw_spec = dict(spec, lower=raw_lo, upper=raw_hi, groups=raw_groups)
    ...
    detail = dict(
        ...
        global_band=global_band, statistical_global_band=statistical_global_band,
        physical_node0_global_band=physical_global_band,
        global_band_tightened_by_node0_physics=not np.allclose(global_band, statistical_global_band, atol=TOL, rtol=0),
        fleet_size=fleet_size, regional_bands=np.column_stack([region_lower, region_upper]),
...
def main():
    ...
    source_paths = [SOURCE/"manifest.json", SOURCE/"protocol_freeze.json", DATASET_METADATA, PREDICTION_ARCHIVE]
    for case in cases:
    ...
    original_protocol = read(SOURCE/"protocol_freeze.json")
    expected_archive_hash = next((value for name, value in original_protocol["source_sha256"].items()
                                  if Path(name).resolve() == PREDICTION_ARCHIVE.resolve()), None)
    if expected_archive_hash != hashes_before[str(PREDICTION_ARCHIVE)]:
        raise AssertionError("Frozen prediction archive differs from the inherited source")
    fleet_size = float(read(DATASET_METADATA)["fleet_size"])
    if not np.isfinite(fleet_size) or fleet_size <= 0: raise ValueError("Invalid source fleet size")
    with np.load(PREDICTION_ARCHIVE, allow_pickle=False) as archive:
        source_prediction = archive["prediction"].copy()
        source_sample_ids = archive["sample_id"].copy().reshape(-1)
    source_positions = {}
    for case in cases:
        matches = np.flatnonzero(source_sample_ids == case["sample_id"])
        if len(matches) != 1: raise AssertionError("Prediction sample identifier is not unique")
        position = int(matches[0]); source_positions[case["sample_id"]] = position
        full_prediction = source_prediction[position, :, 0]
        if np.any(full_prediction < -TOL) or abs(float(full_prediction.sum())-fleet_size) > TOL:
            raise AssertionError("Source node0 plus station prediction does not match the metadata fleet")
    ...
    protocol = dict(
        ...
        station_count=567, vehicle_count=4, truck_capacity=30, fleet_size=fleet_size,
        ...
        geography=geography,
        uncertainty_rule="nested shared box; C0 adds global statistical band intersected with physical 0-node inventory [0,F]; C0V adds four disjoint signed regional sums",
        node0_physics="C0/C0V: -sum(p') <= sum(epsilon') <= F-sum(p'); B intentionally omits global coupling",
        node0_bound_amendment="Previous pre-outcome campaign omitted node0 inventory bound; 108/144 global bands permitted station total above fleet. Superseded before actual access; correction is physical validity, not outcome-based tuning.",
        fleet_source=dict(metadata=str(DATASET_METADATA), prediction_archive=str(PREDICTION_ARCHIVE),
                          source_prediction_members_opened=["prediction", "sample_id"],
                          full_prediction_totals_verified_cases=144, actual_opened=False),
        ...
        source_sha256=hashes_before, original_protocol=original_protocol,
    ...
    for case in cases:
        ...
        try:
            ...
            position = source_positions[case["sample_id"]]
            if not np.array_equal(old_predicted, source_prediction[position, 1:, 0]):
                raise AssertionError("Original station prediction differs from frozen source")
            clipped = np.clip(old_predicted, 0, data["station_capacity"])
            if clipped.sum() > fleet_size+TOL: raise ValueError("D nominal station inventory exceeds fleet")
            data["fleet_size"] = np.asarray(fleet_size)
            data["node0_predicted_original"] = np.asarray(source_prediction[position, 0, 0], float)
            data["node0_nominal_after_center_shift"] = np.asarray(fleet_size-clipped.sum())
            ...
            metadata.update(
                ...
                arms=["B", "C0", "C0V", "D"],
                fleet_size=fleet_size, node0_physical_global_bound_enforced_in=["C0", "C0V"],
            ...
            records.append(dict(case=name, sample_id=case["sample_id"], origin=metadata["origin"],
                                ...
                                node0_band_tightened=calibration["global_band_tightened_by_node0_physics"],
    ...
    index = dict(cases=records, count=len(records), station_count=567,
                 ...
                 node0_physical_band_tightened_cases=sum(r["node0_band_tightened"] for r in records),
